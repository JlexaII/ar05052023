document.addEventListener("DOMContentLoaded", function () {
    ...
    t = jQuery, a = {
        ...
        deleteProducts: function (a, s) {
            ...
            }).html('<span class="smart-basket__delete-icon">Ã</span>');
        ...
        showProducts: function (s) {
            let r = t("<form>").attr({
                ...
                }).html("Ð¦ÐµÐ½Ð° Ð·Ð° 1 ÑÐ¾Ð²Ð°Ñ Ð¸Ð»Ð¸ ÑÑÐ»ÑÐ³Ñ").append(p),
                u = t("<div>").attr({
                    class: "smart-basket__product-price"
                }).html(`Ð¦ÐµÐ½Ð° / ${a.smartBasketCurrency}`).append(c).append(d);
            c.click(() => {
                d.fadeToggle()
            }), p.click(t => {
                t.preventDefault(),
            ...
            }).html(`ÐÐ±ÑÐ°Ñ ÑÐµÐ½Ð° / ${a.smartBasketCurrency}`),
                k = t("<div>").attr({
                    class: "smart-basket__product-delete"
                }).text("Ð£Ð´Ð°Ð»Ð¸ÑÑ");
            ...
            for (let n in s) {
                ...
                for (let a in s[n]) {
                    ...
                    if ("sbName" === a && (c.append(`<span>${s[n][a]}</span>`), p.val(s[n][a])), "sbSize" === a && void 0 !== s[n][a] && (c.children("span").append(`<span class="smart-basket__product-size"> Ð Ð°Ð·Ð¼ÐµÑ: ${s[n][a]}</span>`), d.val(s[n][a])), "sbPrice" === a && (_.html(s[n][a]), h.val(s[n][a])), "sbPriceCommon" === a) {
                        let t = +s[n][a];
                        y.val(+t.toFixed(2))
                    }
        ...
        stateBasket: function () {
            ...
            } else console.log("ÐÐ°Ð¿Ð¾Ð»Ð½Ð¸ÑÐµ Ð¿Ð°ÑÐ°Ð¼ÐµÑÑÑ productSize Ð¸ productPrice");
            return e.getSmartBasketMinState(o), c.click(function () {
                let r = e.getProducts() || {},
                    n = t(this).attr("data-sb-id-or-vendor-code");
                if (void 0 !== r[n]) {
                    let a = t(this).html();
                    return t(this).text("Ð¢Ð¾Ð²Ð°Ñ ÑÐ¶Ðµ Ð² ÐºÐ¾ÑÐ·Ð¸Ð½Ðµ"), t("body").append(e.alertBlock("alreadyAdded")), setTimeout(() => {
                        t(this).html(a)
                    }, 1500), !1
                } {
                    let i = {};
        ...
        commonResult: function (e, s, r) {
            let n = 0,
                ...
                }).text("ÐÐ¾ÑÐ·Ð¸Ð½Ð° Ð¿ÑÑÑÐ°. ÐÑ Ð½Ðµ Ð´Ð¾Ð±Ð°Ð²Ð¸Ð»Ð¸ Ð½Ð¸ Ð¾Ð´Ð½Ð¾Ð³Ð¾ ÑÐ¾Ð²Ð°ÑÐ°").fadeOut(),
                l = t("<div>").attr({
                    class: "smart-basket__success-title"
                }).text("ÐÐ°ÐºÐ°Ð· Ð¿ÑÐ¸Ð½ÑÑ. ÐÐ¶Ð¸Ð´Ð°Ð¹ÑÐµ Ð·Ð²Ð¾Ð½ÐºÐ°").css("display", "none"),
                c = t("<div>").attr({
            ...
            if (0 == +n ? o.fadeIn() : o.fadeOut(), r) t("." + l.attr("class")).css("display", "block"), setTimeout(() => {
                t("." + l.attr("class")).css("display", "none"),
                    t("." + o.attr("class")).css("display", "block")
            }, 3e3);
            else {
                if (!s) return d.html(`<span>ÐÑÐµÐ³Ð¾ ÑÐ¾Ð²Ð°ÑÐ¾Ð²: </span> ${n}`), p.html(`<span>ÐÐ±ÑÐ°Ñ ÑÑÐ¾Ð¸Ð¼Ð¾ÑÑÑ: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`), c.append(o).append(l).append(d).append(p), c;
                0 == +n && t("." + o.attr("class")).css("display", "block"), t("." + d.attr("class")).html("<span>ÐÑÐµÐ³Ð¾ ÑÐ¾Ð²Ð°ÑÐ¾Ð²: </span> "), t("." + d.attr("class")).html(`<span>ÐÑÐµÐ³Ð¾ ÑÐ¾Ð²Ð°ÑÐ¾Ð²: </span> ${n}`), t("." + p.attr("class")).html("<span>ÐÐ±ÑÐ°Ñ ÑÑÐ¾Ð¸Ð¼Ð¾ÑÑÑ: </span> "), t("." + p.attr("class")).html(`<span>ÐÐ±ÑÐ°Ñ ÑÑÐ¾Ð¸Ð¼Ð¾ÑÑÑ: </span> <input name="finalPrice" class="smart-basket__total-cost" type="text" value="${i.toFixed(2)} ${a.smartBasketCurrency}">`)
            }
        },
        ...
        userForm: function () {
            ...
            let c = t("<div>").attr({
                ...
                p = t("<input>").attr({
                    class: "smart-basket__user-input",
                    type: "email",
                    placeholder: "ÐÐ²ÐµÐ´Ð¸ÑÐµ e-mail",
                    name: "userEmail",
            ...
            if (c.append(p), a.agreement.isRequired) {
                ...
                let e = t("<label>").attr({
                    ...
                    s = t("<input>").attr({
                        ...
                        value: "ÐÐ¿Ð»Ð°ÑÐ¸ÑÑ ÑÐµÐ¹ÑÐ°Ñ"
                    }),
                    r = null;
                r = a.agreement.isLink ? t("<a>").attr({
                    ...
                    class: "smart-basket__link-agreement"
                }), s.is(":checked") ? r.text("ÐÐ¿Ð»Ð°ÑÐ¸ÑÑ ÑÐµÐ¹ÑÐ°Ñ") : r.text("Ð¯ Ð½Ðµ Ð¿ÑÐ¸Ð½Ð¸Ð¼Ð°Ñ ÑÑÐ»Ð¾Ð²Ð¸Ñ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÑÑÐºÐ¾Ð³Ð¾ ÑÐ¾Ð³Ð»Ð°ÑÐµÐ½Ð¸Ñ"), e.click(function () {
                    s.is(":checked") ? (r.text("Ð¯ Ð½Ðµ Ð¿ÑÐ¸Ð½Ð¸Ð¼Ð°Ñ ÑÑÐ»Ð¾Ð²Ð¸Ñ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÑÑÐºÐ¾Ð³Ð¾ ÑÐ¾Ð³Ð»Ð°ÑÐµÐ½Ð¸Ñ"), s.val("Ð¯ Ð½Ðµ Ð¿ÑÐ¸Ð½Ð¸Ð¼Ð°Ñ ÑÑÐ»Ð¾Ð²Ð¸Ñ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÑÑÐºÐ¾Ð³Ð¾ ÑÐ¾Ð³Ð»Ð°ÑÐµÐ½Ð¸Ñ"), m.attr({
                        disabled: "disabled"
                    }), d.addClass("smart-basket__input-wrapper_error")) : (r.text("ÐÐ¿Ð»Ð°ÑÐ¸ÑÑ ÑÐµÐ¹ÑÐ°Ñ"), s.val("ÐÐ¿Ð»Ð°ÑÐ¸ÑÑ ÑÐµÐ¹ÑÐ°Ñ"), m.attr({
                        disabled: !1
        ...
        return e[a] ? e[a].apply(this, Array.prototype.slice.call(arguments, 1)) : "object" != typeof a && a ? void t.error("ÐÐµÑÐ¾Ð´ " + a + " Ð½Ðµ Ð½Ð°Ð¹Ð´ÐµÐ½") : e.init.apply(this, arguments)
     }
});